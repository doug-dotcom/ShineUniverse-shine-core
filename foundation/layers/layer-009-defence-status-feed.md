...
The service evaluates the app-supplied commit/profile identity against Core's reviewed ecosystem ledger and current append-only revocation ledger.
...
Node tests cover all release states, revocation precedence, malformed requests, public unauthenticated access, no-store responses and the existing authenticated access-evaluation boundary.

Deno checks that the production Edge Function keeps the Layer 8 federated identity runtime while also loading the canonical Defence review and revocation ledgers for the public status feed.
